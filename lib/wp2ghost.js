...
exports.fromStream = function(stream) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    xml.on('endElement: wp:category', function(category) {
      ...
      if (tag.slug.toLowerCase() in termname2tag) return;

      tags.push(tag);
      termname2tag[tag.slug.toLowerCase()] = tag.id;
    });
    ...
    xml.on('endElement: wp:wp_author', addAuthor);
    xml.on('endElement: wp:author', addAuthor);
    ...
    xml.on('endElement: item', function(item) {
      ...
      if (typeof item.category !== "undefined") {
        for (var i = 0; i < item.category.length; i++) {
          ...
          var name = item.category[i].$.nicename;
          if (name == "DefaultCategory") continue;  // skip, modify by aimingoo.

          posts_tags.push({
            "tag_id": termname2tag[name.toLowerCase()],
            "post_id": post.id
          });
        }
      }
